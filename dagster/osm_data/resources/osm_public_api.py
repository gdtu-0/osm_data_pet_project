...
class OsmPublicApi(ConfigurableResource):
    ...
    def get_closed_changesets_by_bbox(self, min_lon, min_lat, max_lon, max_lat, load_from_ts: Optional[datetime] = None) -> DataFrame:
        ...
        If parameter 'load_from_ts' is not passed, changesets are queried from current time
        in reversed order (newest changesets first)

        If parameter 'load_from_ts' is passed, changesets are queried from timestamp value
        in 'load_from_ts' id direct order (oldest changesets first)
        """
